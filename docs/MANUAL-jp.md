...
各クラスタについて、**力の閉合 (force closure) が成立するまで** 下流側のセル (柱状体) を
追加していきます。閉合判定は次の 3 要素の**ベクトル和** (x, y 成分) で行います
([forces.py:138](../python/region3d/forces.py), [region_grow.py:260](../python/region3d/region_grow.py))。

1. **セルの正味力 ΣQ**: クラスタ内の各柱状体の Q ((a) と同じ式) を x, y 成分に分解して合計。
   セルごとの力は「そのセルのすべり面最大傾斜方向」を向く (後述の回転探索で一律に振る)。
2. **境界プリズム (wedge) の力**: クラスタ外周に張り付けた三角柱列の正味力 (下記)。
3. **根系抵抗 F_roots**: 境界長 × S_roots。閉合誤差から差し引く。

閉合誤差 `err = |ΣQ_セル + ΣQ_プリズム| − F_roots` が
**クラスタ総重量 (プリズム込み) の `err_percent_allowable` % (既定 1%) 以下**になれば安定と
みなして確定します。不成立ならクラスタ下流側の柱状体を追加して再評価し、
`max_growth_cycles` (既定 120) 超過・err の増加傾向 (`err_increase_thresh`) などで打ち切ります。
成長方向が `no_grow` セルに当たった場合、その方向には伸ばしません。

#### (c-1) 境界プリズム — 全周配置と主働/受動土圧

プリズムはクラスタ外周の**全周**に配置されます。外周を閉ループとしてトレースし
([boundary.py:494](../python/region3d/boundary.py))、隣接する境界頂点の全ペアに三角柱を
1 つずつ作ります。主働・受動の区別は、**くさび底面の傾斜角 α をすべり方向との方位差 ω で
連続内挿**することで表現します ([boundary.py:534-543](../python/region3d/boundary.py)、
原論文 式(14)-(15)):

```
ω = (重心→境界セル方向) − すべり方向 DOS
ω = 0°   (末端 toe):    α = 45° − φ/2   … 受動 (Rankine)。長く体積の大きいくさび
ω = 180° (頭部 head):   α = 45° + φ/2   … 主働 (Rankine)。短く急なくさび
中間 (側方):             線形内挿
```

各プリズムの正味力は柱状体と同じ Janbu 型の底面平衡で**符号つき**で計算され
([forces.py:94](../python/region3d/forces.py))、力はクラスタ内向きを正とするため、
**頭部では主働土圧的な駆動力、末端では受動土圧的な抵抗力**として自動的に効きます。
地震慣性も方位差に応じて `k·(−cos ω)` で配分され、頭部で駆動を増やし末端で受動抵抗を
減らします。すべり方向 DOS はセル正味力の合力方位 `atan2(ΣQy, ΣQx)` です。
なお原論文は、この内挿が簡略化であり T 字型クラスタ等では末端以外のプリズムも
受動土圧を発生させ得ることを注記しています。

#### (c-2) 回転探索 — すべり方向の不確かさ (±20°、8 通り)

閉合判定は 1 回ではなく、**すべり移動方向を振った 8 通りの力場**で行い、
最も釣り合いに近いものを採用します ([forces.py:186](../python/region3d/forces.py) `project_slope`)。

- 各セルの力の向きを「**そのセル自身の最大傾斜方位 + rot**」に回す。全セルに同じ rot を
  加えるため、セル間の方位差 (谷型地形の収束パターンなど) は保たれたまま場全体が振れる。
  **共通の絶対方位に揃える操作ではない**。
- 回した方向の見かけ勾配は |∇z|·cos(rot) に減る (斜め下りは坂が緩い)。
- rot は `linspace(−20°, +20°, 8)` の 8 値 (±2.9°, ±8.6°, ±14.3°, ±20°。**0° は含まれない**。
  0° の力場は (a) の不安定セル判定にのみ使用)。
- 8 通りの閉合誤差の**最小値**で判定する = 「どれか 1 つの方向で釣り合えば安定」。

この手続きは原論文 (Mathews et al. 2024) が「すべり方向の不確かさを考慮するため」として
導入した RegionGrow3D 固有のもので、力学的導出や先行研究の引用はなく、±20° は
`--rot_range` / `--rot_num` で変更可能な裁量パラメータです。3D 極限平衡法の文献で
単一のすべり方向を未知数として解く定式 (Lam & Fredlund 1993 など) とは異なります。
...
| α [m⁻¹] | 3.0 | **0.948** |
| K [m² yr⁻¹] | 0.005 | **0.005** (包絡線 3.5–6.5×10⁻³) |
| 輸送則 | 非線形 (S_c=1.25) | **線形** (S_c 不使用) |
| ρ_soil [kg m⁻³] | 1200 | **1090** |
| W_soil [g m⁻² yr⁻¹] | 0 | **0**（原論文が明示的に無視） |
| 土層厚の測り方 | 鉛直 h | **斜面法線 H = h·cosθ** |

`matsushi` は **松四雄騎・外山 真・松崎浩之・千木良雅弘 (2016)「土層の生成および輸送速度の決定と
土層発達シミュレーションに基づく表層崩壊の発生場および崩土量の予測」地形 37(4), 427–453** が
京都近郊の花崗岩流域で宇宙線生成核種 ¹⁰Be により較正した実測値です。D₀・α は p.440 の式(5)と
Fig.7、K は p.441 の Fig.6B（包絡線の算術平均 5×10⁻³ を採用）、ρ_soil は平均乾燥単位体積重量
(p.442)。元素溶脱 W と風成物・有機物の付加 S は「急勾配斜面では |S−W| ≪ |D|」として無視されて
います (p.429)。原論文のシミュレーションは 1 m グリッド・初期土層厚 0.5 m 一様・500 年で、
凸形尾根型斜面 0.3–0.5 m で定常、谷頭凹地は 300–400 年で 0.5→約 1 m、表層崩壊の再来間隔
700–800 年という結果です。同論文の原位置試験値は c = 1.43 kPa、φ = 29.1°、飽和/乾燥単位体積重量
15.4 / 10.7 kN m⁻³、飽和透水係数 2×10⁻⁴ m s⁻¹。

> ⚠ **このパラメータを能登の 5 m DEM に適用しても、尾根は薄くなりません。** 能登の凸型セルの
> 曲率は中央値 0.0195 m⁻¹（5 m スケール）ですが、上記パラメータで定常土層厚 0.4 m を得るには
> 曲率 0.121 m⁻¹ が必要で、これに達するのは凸型セルの **0.14%** だけです。曲率 0.02 m⁻¹ での
> 定常解は 2.30 m になります。つまり能登の地形では生成が輸送を大きく上回る production-limited
> な状態で、原論文と同じ設定（初期 0.5 m・500 年）で回すと**ほぼ一様に 0.75 m 前後**（尾根 0.66 m、
> 平坦 0.75 m、凹地 0.86 m）にしかなりません。これはパラメータの誤りではなくサイトの違いです:
> 原論文 Fig.6B が示す白川流域の Nose の 5 m 曲率は **+0.1〜+0.25 m⁻¹**（回帰 −∇²z＝−0.34H+0.22）
> で、能登より 1 桁鋭い 0 次谷の凸型稜です。また α=0.948 が既往レンジ 1.4–3.8 m⁻¹ より小さく
> D₀ が大きいことは原論文自身が指摘しています (p.440)。定常解を使うなら 2–3 m、原論文設定なら
> 約 0.75 m と結果が大きく変わるため、どちらを採るかは明示して報告してください。
>
> 検証: 本実装の解析解は、原論文の数値スキーム（式(7) の前進オイラー 1 yr × 500 回）と
> 能登 DEM 300×300 セル窓で最大差 0.0001 m で一致することを確認済み (2026-07-28)。

なお **松四雄騎・松崎浩之・牧野久識 (2014) 地形 35(2), 165–185** にも日本の花崗岩流域の値
(S_c ≈ 1、K/L = 7×10⁻⁵–3×10⁻⁴ m yr⁻¹、ρ_s = 1.9×10⁶ g m⁻³、W = 66.8 g m⁻² yr⁻¹、流域削剥速度
2.0×10²–1.8×10⁴ g m⁻² yr⁻¹) がありますが、これは**流域スケールの削剥速度と地形方程式の検証**の
ための値で、土層発達シミュレーション用ではありません。プリセットには使っていません。
...
すると、発散斜面は定常解、収束セルは「最終崩壊から 750 年分の再堆積」として過渡解を使います
...
実行例 — 松四ほか (2016) と同じ設定 (初期 0.5 m 一様・500 年、能登 5 m DEM 6,193 万セル、約 67 秒):
...
能登での結果 (陸域のみ、海岸線マスク適用): 平均 0.756 m / 中央値 0.749 m / 5–95%点 0.63–0.89 m。
地形別では凸型尾根 0.66 m、平坦 0.75 m、凹地 0.86 m と差は小さく、上記の警告どおり production-
limited です。Roering 5000 yr は平均 1.14 m・中央値 1.06 m・0.5 m 未満 13.8%。
尾根・急崖・谷の断面比較図は `python python/_viz_soil_depth_transects.py` で再生成できます。

> ⚠ 統計や図を出すときは必ず海域を除いてください。この DEM は海面を約 0〜0.5 m で埋めており
> 標高閾値では分離できないため、`lib/coastline/N03-20260101_17.shp` (国土数値情報 行政区域) を
> ラスタ化してマスクします (陸域 56,386,695 セル = 1,410 km²)。成果ラスタ自体は driver が
> DEM 有効セル全域に土層厚を要求するため、海域も値を持たせたままにしてあります。
...
> ⚡ 上表は**計算量**の目安です。「全ラン並列実行」(2本並列) を使うと全分布の
> **実時間 (wall-clock)** は約半分になります（計算量自体は変わりません、§10.2）。

### 10.2 高速化テクニック
1. **Numba 必須**: 入っていないと soil_depth が 50 倍以上遅くなる
2. **mat ソース**: 同じ DEM で再計算しないなら mat にするだけで前処理が秒オーダー
3. **φ 並列 (2本並列)**: 「⚡ 全ラン並列実行」で 10 ラン を 2 プロセス同時に走らせると
   スループット約 2 倍（`_sus_parallel.py`、§3.6）。結果は直列とビット一致。メモリは
   1 ラン約 20 GB（Docker は `.wslconfig` で VM を増やす）
4. **Qhull 高速化 (Windows・自動)**: `region3d/_fastqhull.py` が scipy Delaunay の
   呼び出し毎の一時ファイル生成（Windows で Defender スキャンにより律速。クラスタ境界
   計算の支配的コスト）を単一ファイル再利用で除去 → 1 ラン約 2 倍。出力ビット一致、
   Linux では元々発生しないので no-op
5. **CPU 並列 (1 ラン内)**: 1 ラン内のクラスタループは概ね単スレッド。`prange` 化済の
   Numba 関数のみ自動で並列化
...
- 松四雄騎・松崎浩之・牧野久識 (2014). 宇宙線生成核種による流域削剥速度の決定と
  ...
  → 日本の花崗岩流域の流域スケール値 (S_c≈1、K/L=7×10⁻⁵–3×10⁻⁴ m yr⁻¹、
  W=66.8 g m⁻² yr⁻¹、流域削剥速度 2.0×10²–1.8×10⁴ g m⁻² yr⁻¹)。地形方程式の
  検証用であり `matsushi` プリセットには不使用。J-STAGE で全文無料公開。
- 松四雄騎・外山 真・松崎浩之・千木良雅弘 (2016). 土層の生成および輸送速度の決定と
  土層発達シミュレーションに基づく表層崩壊の発生場および崩土量の予測.
  *地形*, 37(4), 427-453.
  → **`matsushi` プリセットの出典** (白川流域、京都盆地東部の白亜紀花崗岩)。
  D₀=965.8 g m⁻² yr⁻¹・α=0.948 m⁻¹ (p.440 式(5)+Fig.7)、K=5×10⁻³ m² yr⁻¹
  (Fig.6B 包絡線 3.5–6.5×10⁻³ の平均)、ρ_soil=1.09×10⁶ g m⁻³ (p.442)、
  計算式 ∂h/∂t=K∇²z+(D₀/ρ_soil)e^(−αh|cosθ|) (式(7))。原位置試験値
  c=1.43 kPa、φ=29.1°、γ_sat/γ_dry=15.4/10.7 kN m⁻³、K_sat≈2×10⁻⁴ m s⁻¹
  (Table 3, Fig.8)。全パラメータを原文の頁・図と照査済み (2026-07-28)。

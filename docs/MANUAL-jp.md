...
> 平坦 0.75 m、凹地 0.86 m）にしかなりません。松四らの薄い尾根は、1 m LiDAR で捉えた 0 次谷の
> 鋭い凸型稜（曲率 0.1 m⁻¹ 級）に由来するもので、5 m DEM の能登丘陵には直接転用できません。
> 定常解を使うなら 2–3 m、原論文設定なら約 0.75 m、と結果が大きく変わるため、どちらを採るかは
> 明示して報告してください。
